...
contract LiquidationOperator3 is IUniswapV2Callee {
    ...
    function uniswapV2Call(
        ...
        console.log("Flash Loan USDC: ", USDC.balanceOf(address(this))); //เงิน USDC ที่ไปกู้มา (flash loan)

        // 2.1 liquidate the target user
        console.log("----> liquidate the target user");
        uint debtToCover = amount1; // USDC ที่จะนำไป liquidate
        USDC.approve(address(lendingPool), debtToCover);
        lendingPool.liquidationCall(address(WETH), address(USDC), liquidationTarget, debtToCover, false); //liquidation
        uint collateral_WETH = WETH.balanceOf(address(this));
        console.log("Collateral WETH: ", collateral_WETH); //ได้ WETH กลับมา

        // 2.3 repay

        uint repay_WETH = getAmountIn(debtToCover, reserve_WETH_Pool1, reserve_USDC_Pool1);
        console.log("WETH for repay: ", repay_WETH);

        if(collateral_WETH > repay_WETH)
            console.log("Profit: ",collateral_WETH-repay_WETH);
        else
            console.log("Loss: ",repay_WETH-collateral_WETH);

        console.log("----> repay flash loan");
        WETH.transfer(address(uniswapV2Pair_WETH_USDC), repay_WETH); // คืน WETH เข้าไปใน pool
