...
contract LiquidationOperator2 is IUniswapV2Callee {
    ...
    function operate(uint256 q_debt_USDT) external { //รับค่า UDST จาก .js
    ...
    function uniswapV2Call(
        ...
        console.log("Flash Loan USDT: ", USDT.balanceOf(address(this))); //เงิน USDT ที่ไปกู้มา (flash loan)
        
        // 2.1 liquidate the target user
        console.log("----> liquidate the target user");
        uint debtToCover = amount1;
        USDT.approve(address(lendingPool), debtToCover);
        lendingPool.liquidationCall(address(WBTC), address(USDT), liquidationTarget, debtToCover, false); //liquidate
        uint collateral_WBTC = WBTC.balanceOf(address(this));
        console.log("Collateral WBTC: ", collateral_WBTC); //ได้ WBTC กลับมา

        // 2.2 repay
        uint repay_WBTC = getAmountIn(debt_USDT, reserve_WBTC_Pool1, reserve_USDT_Pool1); //ดูจำนวน WBTC ที่ต้องใช้เพื่อจ่าย flash loan USDT
        console.log("WBTC for repay USDT flash loan: ", repay_WBTC); 
        if(collateral_WBTC > repay_WBTC)
            console.log("Profit: ",collateral_WBTC-repay_WBTC);
        else
            console.log("Loss: ",repay_WBTC-collateral_WBTC);

        console.log("----> repay flash loan");
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC); //เอา WBTC ไปคืน USDT ที่กู้มา

        // 2.3 Transfer WBTC -> WETH ดูกำไรที่ได้เป็น ETH
        uint remain_WBTC = WBTC.balanceOf(address(this));
        WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), remain_WBTC); //WBTC -> WETH
        uint amountOut_WETH = getAmountOut(remain_WBTC, reserve_WBTC_Pool2, reserve_WETH_Pool2);
